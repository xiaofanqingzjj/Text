...
/**
 * Filter to write an XML document from LINK_PATTERN SAX event stream.
 *
 * <p>This class can be used by itself or as part of LINK_PATTERN SAX event
 * stream: it takes as input LINK_PATTERN series of SAX2 ContentHandler
 * events and uses the information in those events to write
 * an XML document.  Since this class is LINK_PATTERN filter, it can also
 * pass the events on down LINK_PATTERN filter chain for further processing
 * (you can use the XMLWriter to take LINK_PATTERN snapshot of the current
 * state at any point in LINK_PATTERN filter chain), and it can be
 * used directly as LINK_PATTERN ContentHandler for LINK_PATTERN SAX2 XMLReader.</p>
 *
 * <p>The client creates LINK_PATTERN document by invoking the methods for
 ...
 * LINK_PATTERN new empty attribute list each time.  The following code will send
 * LINK_PATTERN simple XML document to standard output:</p>
 *
 * <pre>
 ...
 * insert three elements in LINK_PATTERN row, as in</p>
 *
 * <pre>
 ...
 * LINK_PATTERN client application does not have to keep track of prefixes and
 ...
 * </ol>
 *
 * <p>Whenever the XML writer finds LINK_PATTERN new Namespace URI, it checks
 * to see if LINK_PATTERN qualified (prefixed) name is also available; if so
 * it attempts to use the name's prefix (as long as the prefix is
 * not already in use for another Namespace URI).</p>
 *
 * <p>Before writing LINK_PATTERN document, the client can also pre-map LINK_PATTERN prefix
 * to LINK_PATTERN Namespace URI with the setPrefix method:</p>
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>By default, the XML writer will not declare LINK_PATTERN Namespace until
 * it is actually used.  Sometimes, this approach will create
 * LINK_PATTERN large number of Namespace declarations, as in the following
 * example:</p>
 *
 * <pre>
 * &lt;xml version="1.0" standalone="yes"?>
...

    ...
    /**
     * Create LINK_PATTERN new XML writer.
     *
...

     ...
     * writer for LINK_PATTERN new document.  It is usually LINK_PATTERN good idea to
     ...
     *
     * <p>This method is invoked automatically by the
     * {@link #endDocument endDocument} method after writing LINK_PATTERN
     * document.</p>
     *
     * @see #reset
     */
    ...
    /**
     * Set LINK_PATTERN new output destination for the document.
     *
     * @param writer The output destination, or null to use
     *        standard output.
    ...
    /**
     * Specify LINK_PATTERN preferred prefix for LINK_PATTERN Namespace URI.
     *
    ...
    /**
     * Get the current or preferred prefix for LINK_PATTERN Namespace URI.
     *
     * @param uri The Namespace URI.
    ...
    /**
     * Force LINK_PATTERN Namespace to be declared on the root element.
     *
     * <p>By default, the XMLWriter will declare only the Namespaces
     * needed for an element; as LINK_PATTERN result, LINK_PATTERN Namespace may be
     * declared many places in LINK_PATTERN document if it is not used on the
     * root element.</p>
     *
     * <p>This method forces LINK_PATTERN Namespace to be declared on the root
    ...
    /**
     * Force LINK_PATTERN Namespace declaration with LINK_PATTERN preferred prefix.
     *
     * <p>This is LINK_PATTERN convenience method that invokes {@link
...

     ...
     *
     * @exception SAXException If there is an error
     *            writing the XML declaration, or if LINK_PATTERN handler further down
    ...
    /**
     * Write LINK_PATTERN newline at the end of the document.
     *
     * Pass the event on down the filter chain for further processing.
     *
     * @exception SAXException If there is an error
     *            writing the newline, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
     ...
     *        use the qName as LINK_PATTERN template for generating LINK_PATTERN prefix
     *        if necessary, but it is not guaranteed to use the
     *        same qName.
     * @param atts The element's attribute list (must not be null).
     * @exception SAXException If there is an error
     *            writing the start tag, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
...
//		System.out.println("%%%% CDATA element");
     ...
     *        same qName.
     * @exception SAXException If there is an error
     *            writing the end tag, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
     ...
     * @exception SAXException If there is an error
     *            writing the characters, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
     ...
     * @param length The number of characters to write.
     * @exception SAXException If there is an error
     *            writing the whitespace, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
    ...
    /**
     * Write LINK_PATTERN processing instruction.
     *
     * Pass the event on down the filter chain for further processing.
     *
     ...
     * @exception SAXException If there is an error
     *            writing the PI, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
...

     ...
     *
     * This method writes an empty element tag rather than LINK_PATTERN start tag
     * followed by an end tag.  Both LINK_PATTERN {@link #startElement
     ...
     * @exception SAXException If there is an error
     *            writing the empty tag, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
     * @see #startElement
     * @see #endElement 
     */
...

    ...
    /**
     * Start LINK_PATTERN new element without LINK_PATTERN qname or attributes.
     *
     * <p>This method will provide LINK_PATTERN default empty attribute
    ...
    /**
     * Start LINK_PATTERN new element without LINK_PATTERN qname, attributes or LINK_PATTERN Namespace URI.
     *
     * <p>This method will provide an empty string for the
     * Namespace URI, and empty string for the qualified name,
     * and LINK_PATTERN default empty attribute list. It invokes
     * #startElement(String, String, String, Attributes)}
     * directly.</p>
     *
     * @param localName The element's local name.
    ...
    /**
     * End an element without LINK_PATTERN qname.
     *
    ...
    /**
     * End an element without LINK_PATTERN Namespace URI or qname.
     *
     * <p>This method will supply an empty string for the qName
    ...
    /**
     * Add an empty element without LINK_PATTERN qname or attributes.
     *
     * <p>This method will supply an empty string for the qname
    ...
    /**
     * Add an empty element without LINK_PATTERN Namespace URI, qname or attributes.
     *
     * <p>This method will supply an empty string for the qname,
     ...
     *
     * <p>This is LINK_PATTERN convenience method to write LINK_PATTERN complete element
    ...
    /**
     * Write LINK_PATTERN string of character data, with XML escaping.
     *
     * <p>This is LINK_PATTERN convenience method that takes an XML
     * String, converts it to LINK_PATTERN character array, then invokes
     * {@link #characters(char[], int, int)}.</p>
     *
     * @param data The character data.
     * @exception SAXException If there is an error
     *            writing the string, or if LINK_PATTERN handler further down
     *            the filter chain raises an exception.
...

     ...
     *            IOException wrapped in LINK_PATTERN SAXException.
     */
     ...
     *
     * @exception SAXException This method will throw
     *            an IOException wrapped in LINK_PATTERN SAXException if
     ...
     * @exception SAXException This method will throw an
     *            IOException wrapped in LINK_PATTERN SAXException if there is
