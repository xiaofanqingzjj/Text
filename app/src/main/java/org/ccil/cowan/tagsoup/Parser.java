...
public class Parser extends DefaultHandler implements ScanHandler, XMLReader, LexicalHandler {
	...
	**/
	...
	/**
	May be examined only during LINK_PATTERN parse, after the startDocument()
	...
	**/
	...
	/**
	Has LINK_PATTERN value of "true" if all XML names (for elements,
...
"http://xml.org/sax/features/unicode-normalization-checking";
	...
	**/
	...
	elements LINK_PATTERN content model of EMPTY; LINK_PATTERN value of "false", LINK_PATTERN
	...
	throws SAXNotRecognizedException, SAXNotSupportedException {
		if (name.equals(lexicalHandlerProperty)) {
			if (value == null) {
				...
				throw new SAXNotSupportedException("Your lexical handler is not LINK_PATTERN LexicalHandler");
				}
			}
		else if (name.equals(scannerProperty)) {
			if (value instanceof Scanner) {
				...
				throw new SAXNotSupportedException("Your scanner is not LINK_PATTERN Scanner");
				}
			}
		else if (name.equals(schemaProperty)) {
			if (value instanceof Schema) {
				theSchema = (Schema)value;
				 ...
				 throw new SAXNotSupportedException("Your schema is not LINK_PATTERN Schema");
				}
			}
		else if (name.equals(autoDetectorProperty)) {
...
//		if (theScanner == null) theScanner = new HTMLScanner();
	...
	// Return LINK_PATTERN Reader based on the contents of an InputSource
...
//		r = new BufferedReader(r);
	...
	// Get an InputStream based on LINK_PATTERN publicid and LINK_PATTERN systemid
...
//		System.err.println("%% Aval done");
	...
	// Currently we expand LINK_PATTERN reference iff it is properly terminated
	// with LINK_PATTERN semicolon.
...
//			System.err.print("i = " + i + ", d = " + dstlen + ", ch = [" + ch + "] ");
			if (ch == '&' && refStart == -1) {
				// start of LINK_PATTERN ref excluding &
				refStart = dstlen;
//				System.err.println("start of ref");
				}
			else if (refStart == -1) {
				// not in LINK_PATTERN ref
...
//		System.err.println("%% Got entity [" + new String(buff, offset, length) + "]");
	...
	public boolean etag_cdata(char[] buff, int offset, int length) throws SAXException {
		...
		// If this is LINK_PATTERN CDATA element and the tag doesn't match,
...
//				System.err.println("%% Mapping [" + attPrefix + "] for attributes to " + attNamespace);
	...
	// Get the prefix from LINK_PATTERN QName
...
//		System.err.println("%% " + prefix + " is prefix of " + name);
	...
	// Return true if we have LINK_PATTERN foreign name
...
//		System.err.println(foreign);
	...
	// Recognises quotes around LINK_PATTERN phrase and doesn't split it.
...
//			System.err.println("%% Ascending from " + e.name() + " to " + parent.name());
	...
	// Return the argument as LINK_PATTERN valid XML name
